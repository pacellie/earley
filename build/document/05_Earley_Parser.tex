...
\begin{itemize}
  ...
  \item A predecessor pointer \isa{Pre\ pre} of an entry $e$ is sound for the input \isa{{\isasymomega}}, bins \isa{bs},
    ...
    then item of the entry $e$. Item $x'$ \isa{scans} item $x$ for index $k$ if the next symbol of
  ...
  \item We define the soundness a pointer \isa{PreRed\ p\ ps} of an entry $e$ for each predecessor/reduction
...
The function \isa{build{\isacharunderscore}{\kern0pt}tree{\isacharprime}{\kern0pt}} constructs the \textit{single} parse tree corresponding to the
...
\isa{{\isacharparenleft}{\kern0pt}k{\isacharprime}{\kern0pt}{\isacharcomma}{\kern0pt}\ pre{\isacharcomma}{\kern0pt}\ red{\isacharparenright}{\kern0pt}} since we are only constructing a single derivation tree. As for the predecessor
...
\begin{isamarkuptext}%
Finally, function \isa{build{\isacharunderscore}{\kern0pt}tree} computes a complete derivation tree if there exists one. It searches the last bin for any finished items or items of the form
...
the reduction triple. Furthermore, the implementation of function \isa{bin{\isacharunderscore}{\kern0pt}upd} also makes sure to not change this
...
We assume that the bins (or argument \isa{bs}) are valid according to Chapter \ref{chapter:3} and
contain only sound and monotonic pointers. The algorithm \isa{build{\isacharunderscore}{\kern0pt}tree{\isacharprime}{\kern0pt}} calls itself at most
