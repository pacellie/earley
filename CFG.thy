...
definition valid_startsymbol :: "'a cfg \<Rightarrow> bool" where
  "valid_startsymbol cfg \<equiv> \<SS> cfg \<in> set (\<NN> cfg)"

definition valid_rules :: "'a cfg \<Rightarrow> bool" where
  "valid_rules cfg \<equiv> \<forall>(N, \<alpha>) \<in> set (\<RR> cfg). N \<in> set (\<NN> cfg) \<and> (\<forall>s \<in> set \<alpha>. s \<in> set (\<NN> cfg) \<union> set (\<TT> cfg))"
...
definition wf_cfg :: "'a cfg \<Rightarrow> bool" where
  "wf_cfg cfg \<equiv> disjunct_symbols cfg \<and> valid_startsymbol cfg \<and> valid_rules cfg \<and> distinct_rules cfg"
...
definition wf_sentence :: "'a cfg \<Rightarrow> 'a sentence \<Rightarrow> bool" where
  "wf_sentence cfg s \<equiv> \<forall>x \<in> set s. is_symbol cfg x"

lemma is_nonterminal_startsymbol:
  "wf_cfg cfg \<Longrightarrow> is_nonterminal cfg (\<SS> cfg)"
