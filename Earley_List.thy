...
subsection \<open>Indexing and Subsetting\<close>
...
lemma wf_bins_app_bins:
  ...
  unfolding wf_bins_def wf_bin_def app_bins_def using set_bin_app_bin
  ...
  proof (cases "\<not> length (items (bins bs ! k)) \<le> i")
    ...
    thus ?thesis
      using True 1 wf_bins_Scan_it wf_bins_Predict_it wf_bins_Complete_it
            wf_bins_app_bins
      by (auto simp: \<pi>_it'.simps Let_def simp del: set_bin_def split: option.split)
...
next
  ...
  moreover have "wf_bins ?bs'"
    using 0 wf_bins_app_bins Complete.hyps(2) Complete.prems(1,4) wf_bins_Complete_it by blast
...
next
  ...
  moreover have "wf_bins ?bs'"
    using 0 wf_bins_Scan_it wf_bins_app_bins Scan.hyps(3,5) Scan.prems(1,4) by simp
...
next
  ...
  moreover have "wf_bins ?bs'"
    using Suc_eq_plus1 Suc_le_eq Suc_le_mono Predict.prems(1,3,4) wf_bins_Predict_it wf_bins_app_bins by presburger
...
proof (induction k bs i rule: \<pi>_it'_induct)
  ...
  have 0: "x \<in> set_bin (bins bs ! k)"
    using Complete.hyps(1,2) by force
  have "wf_bins ?bs'"
    using 0 Complete.prems(1,2) wf_bins_Complete_it wf_bins_app_bins by presburger
...
next
  ...
  have 0: "x \<in> set_bin (bins bs ! k)"
    using Scan.hyps(1,2) by force
  have "wf_bins ?bs'"
    using 0 Scan.prems(1,2) wf_bins_Scan_it wf_bins_app_bins Scan.hyps(3,5) by simp
  moreover have "sound_items (set (Scan_it k a x bs))"
...
next
  ...
  have 0: "x \<in> set_bin (bins bs ! k)"
    using Predict.hyps(1,2) by force
  have "wf_bins ?bs'"
    using 0 Predict.prems(1-3) wf_bins_Predict_it wf_bins_app_bins by simp
  moreover have "sound_items (set (Predict_it k a bs))"
...
proof (induction k bs i rule: \<pi>_it'_induct)
  ...
  thus ?case
    using Scan_bin_absorb Base.hyps Base.prems(4) \<pi>_it'_simps(1) by metis
next
  case (Complete k bs i x)
  let ?bs' = "app_bins bs k (Complete_it k x bs)"
  have "wf_bins ?bs'"
    using Complete.hyps Complete.prems(1,2) wf_bins_Complete_it wf_bins_app_bins set_bin_def by (metis not_le_imp_less nth_mem)
  ...
  proof -
    ...
    also have "... \<subseteq> set_bins bs \<union> Scan k {x}"
      using Complete.prems(3,4) Scan_Un by blast
  ...
  qed
  ultimately have "Scan k (set_bins ?bs') \<subseteq> set_bins (\<pi>_it' k ?bs' (i + 1))"
    using Complete.IH Complete.hyps Complete.prems(2,3) by auto
  thus ?case
    using Complete.hyps Complete.prems(2) set_bins_app_bins Scan_Un by simp
next
  case (Scan k bs i x a)
  let ?bs' = "app_bins bs (k+1) (Scan_it k a x bs)"
  have "wf_bins ?bs'"
    using Scan.hyps Scan.prems(1,2) wf_bins_Scan_it wf_bins_app_bins set_bin_def
    by (metis Option.option.discI less_or_eq_imp_le not_le_imp_less nth_mem)
  moreover have "Scan k (set_bins_upto ?bs' k (i + 1)) \<subseteq> set_bins ?bs'"
  proof -
    have "Scan k (set_bins_upto ?bs' k (i + 1)) = Scan k (set_bins_upto ?bs' k i \<union> {items (bins ?bs' ! k) ! i})"
    ...
    also have "... \<subseteq> set_bins bs \<union> Scan k {x}"
      using Scan.prems(3,4) Scan_Un by blast
  ...
  qed
  ultimately have "Scan k (set_bins ?bs') \<subseteq> set_bins (\<pi>_it' k ?bs' (i + 1))"
    using Scan.IH Scan.prems(2,3) by simp
  thus ?case
    using Scan Scan_Un set_bins_app_bins by auto
next
  case (Pass k bs i x a)
  thus ?case 
...
next
  ...
  have "wf_bins ?bs'"
    using Predict.hyps Predict.prems(1,2,3) wf_bins_Predict_it wf_bins_app_bins set_bin_def by simp
  moreover have "Scan k (set_bins_upto ?bs' k (i + 1)) \<subseteq> set_bins ?bs'"
  proof -
    have "Scan k (set_bins_upto ?bs' k (i + 1)) = Scan k (set_bins_upto ?bs' k i \<union> {items (bins ?bs' ! k) ! i})"
    ...
    also have "... \<subseteq> set_bins bs \<union> Scan k {x}"
      using Predict.prems(3,4) Scan_Un by blast
    also have "... = set_bins bs"
  ...
  qed
  ultimately have "Scan k (set_bins ?bs') \<subseteq> set_bins (\<pi>_it' k ?bs' (i + 1))"
    using Predict.IH Predict.hyps Predict.prems(2,3) by auto
  thus ?case
    using Predict.hyps Predict.prems(2) set_bins_app_bins Scan_Un by simp
qed

lemma Predict_it_sub_\<pi>_it':
  assumes "wf_bins bs" "k < length (bins bs)" "length (bins bs) = length inp + 1"
...
proof (induction k bs i rule: \<pi>_it'_induct)
  ...
  have "bin (set_bins bs) k = bin (set_bins_upto bs k i) k"
    using Base.hyps Base.prems(1,2) bin_set_bins_upto_set_bins_eq by simp
  thus ?case
    using Predict_bin_absorb Base.hyps Base.prems(4) \<pi>_it'_simps(1) by metis
next
  case (Complete k bs i x)
  let ?bs' = "app_bins bs k (Complete_it k x bs)"
  have "wf_bins ?bs'"
  ...
  proof -
    ...
    also have "... \<subseteq> set_bins bs \<union> Predict k {x}"
      using Complete.prems(3,4) Predict_Un by blast
    also have "... = set_bins bs"
      using Complete.hyps(3) Complete.prems(1,2) bin_def kth_bin_in_bins wf_bins_kth_bin Predict_def by auto
    finally show ?thesis
      using Complete.prems(2) set_bins_app_bins by auto
  qed
  ultimately have "Predict k (set_bins ?bs') \<subseteq> set_bins (\<pi>_it' k ?bs' (i + 1))"
    using Complete.IH Complete.hyps Complete.prems(2,3) by auto
  thus ?case
    using Complete.hyps Complete.prems(2) set_bins_app_bins Predict_Un by simp
next
  case (Scan k bs i x a)
  let ?bs' = "app_bins bs (k+1) (Scan_it k a x bs)"
  have "wf_bins ?bs'"
  ...
  proof -
    ...
    also have "... = Predict k (set_bins_upto bs k i \<union> {x})"
      using Scan.hyps(1,2,5) Scan.prems(3) nth_app_bins set_bins_upto_kth_nth_id by simp
    also have "... \<subseteq> set_bins bs \<union> Predict k {x}"
      using Scan.prems(3,4) Predict_Un by blast
    also have "... = set_bins bs"
  ...
  ultimately have "Predict k (set_bins ?bs') \<subseteq> set_bins (\<pi>_it' k ?bs' (i + 1))"
    using Scan.IH Scan.prems(2,3) by simp
  thus ?case
    using Scan Predict_Un set_bins_app_bins by auto
next
  case (Pass k bs i x a)
  ...
  proof -
    ...
    also have "... \<subseteq> set_bins bs \<union> Predict k {x}"
      using Pass.prems(3,4) Predict_Un by blast
    also have "... = set_bins bs"
  ...
  qed
  hence "Predict k (set_bins bs) \<subseteq> set_bins (\<pi>_it' k bs (i + 1))"
    using Pass.IH Pass.prems(1-3) by blast
  thus ?case
    using Pass Predict_Un set_bins_app_bins by auto
next
  case (Predict k bs i x a)
  let ?bs' = "app_bins bs k (Predict_it k a bs)"
  have "wf_bins ?bs'"
  ...
  proof -
    ...
    also have "... = Predict k (set_bins_upto bs k i \<union> {x})"
      using Predict.hyps(1,2) Predict.prems(2) kth_app_bins set_bins_upto_kth_nth_id by simp
    also have "... \<subseteq> set_bins bs \<union> Predict k {x}"
      using Predict.prems(3,4) Predict_Un by blast
  ...
  qed
  ultimately have "Predict k (set_bins ?bs') \<subseteq> set_bins (\<pi>_it' k ?bs' (i + 1))"
    using Predict.IH Predict.hyps Predict.prems(2,3) by simp
  thus ?case
    using Predict.hyps Predict.prems(2) set_bins_app_bins Predict_Un by simp
qed
...
proof standard
  ...
  next
    ...
    have "bin I (item_origin z) \<subseteq> set_bin (bins bs ! item_origin z)"
      using wf_item_in_kth_bin assms(2) assms(4) bin_def by blast
    hence "x \<in> set ?is"
      using "*"(5) B \<open>x \<in> bin I (item_origin y)\<close> set_bin_def by (simp add: in_mono)
    then show ?thesis
...
proof (induction k bs i rule: \<pi>_it'.induct)
  ...
  proof cases
    ...
    moreover have "Complete k (set_bins bs) = Complete k (set_bins_upto bs k i)"
      using a1 "1.prems"(1,2) Q4 by force
    ultimately show ?thesis
  ...
  next
    ...
    proof cases
      ...
      have 1: "sound_items (set (Complete_it k ?x bs))"
        using sound_Complete_it[OF "1.prems"(1) _ x_in_kth_bin "1.prems"(2)]
        by (meson "1.prems"(1,5) set_bins_bin_exists a2 subset_refl wf_bins_def wf_bins_kth_bin wf_items_def)
      have "i < length (items (bins ?bs' ! k))"
        using a1 "1.prems"(2) length_nth_bin_app_bins by (meson less_le_trans not_le_imp_less)
      have "wf_bins ?bs'"
      ...
      proof -
        have 0: "?x = items (bins ?bs' ! k) ! i"
          using "1.prems"(2) kth_app_bins a1 by simp
        ...
        have "Complete k (set_bins_upto ?bs' k (i + 1)) = Complete k (set_bins_upto ?bs' k i \<union> {items (bins ?bs' ! k) ! i})"
          using set_bins_upto_Suc_Un \<open>i < length (items (bins ?bs' ! k))\<close> by simp
        also have "... = Complete k (set_bins_upto bs k i \<union> {?x})"
          using 0 1 by simp
        also have "... \<subseteq> set_bins bs \<union> set (Complete_it k ?x bs)"
          using "1.prems"(4)
          by (metis "1.prems"(1,2) set_bins_upto_sub_set_bins Option.option.discI Q7 a2 next_symbol_def wf_bins_kth_bin x_in_kth_bin)
        finally show ?thesis
    ...
    next
      ...
      proof cases
        ...
        proof cases
          ...
          have 1: "sound_items (set (Scan_it k a ?x bs))"
            using sound_Scan_it[OF "1.prems"(1) _ x_in_kth_bin "1.prems"(2) a_def]
            by (metis "1.prems"(1,4,5) set_bins_bin_exists wf_bins_kth_bin Lattices.semilattice_sup_class.sup.orderE Un_upper1 wf_items_def)
          have "i < length (items (bins ?bs' ! k))"
            using a1 "1.prems"(2) length_nth_bin_app_bins app_bins_def by auto
          have "wf_bins ?bs'"
          ...
          proof -
            have 0: "?x = items (bins ?bs' ! k) ! i"
              using "1.prems"(2) kth_app_bins a1 by (simp add: app_bins_def)
            have 1: "set_bins_upto ?bs' k i = set_bins_upto bs k i"
            ...
            also have "... = Complete k (set_bins_upto bs k i)"
              using Q6 a3 a_def
              by (meson "1.prems"(1,2) set_bins_upto_sub_set_bins set_bins_bin_exists subset_iff wf_bins_def wf_bins_kth_bin wf_items_def x_in_kth_bin)
            also have "... \<subseteq> set_bins bs"
              using "1.prems"(4) by blast
            finally show ?thesis
        ...
        next
          ...
          proof -
            have "Complete k (set_bins_upto bs k (i + 1)) = Complete k (set_bins_upto bs k i \<union> {items (bins bs ! k) ! i})"
              using set_bins_upto_Suc_Un a1 by auto
      ...
      next
        ...
        moreover have "k < length (bins ?bs')"
          using "1.prems"(2) length_bins_app_bins by simp
        moreover have "sound_items (set_bins ?bs')"
          by (metis "1.prems"(1,2,5) sound_Predict_it UnE Un_upper1 a_def set_bins_app_bins sound_items_def x_in_kth_bin)
        moreover have "Complete k (set_bins_upto ?bs' k (i + 1)) \<subseteq> set_bins ?bs'"
        proof -
          have 0: "?x = items (bins ?bs' ! k) ! i"
            using "1.prems"(2) kth_app_bins a1 by simp
          ...
          also have "... = Complete k (set_bins_upto bs k i)"
            ...
            by (metis "local.1.prems"(2) "local.1.prems"(5) set_bins_upto_sub_set_bins in_mono sound_items_def) 
          also have "... \<subseteq> set_bins bs"
            using "1.prems"(4) by blast
          finally show ?thesis
...
lemma ABCD':
  ...
  using assms \<pi>_it'_mono Scan_sub_\<pi>_it' Predict_it_sub_\<pi>_it' D \<pi>_step_def by simp
  ...
  next
    ...
    proof cases
      ...
      have sound: "sound_items (set_bins ?bs')"
        using sound_Complete_it[OF "1.prems"(1) _ x_in_kth_bin "1.prems"(2)]
        by (metis "1.prems"(1,2,5) set_bins_bin_exists UnE a2 set_bins_app_bins sound_items_def subset_refl wf_bins_kth_bin wf_items_def)
      show ?thesis
      proof cases
        assume a3: "i+1 \<le> j"
      ...
      next
        ...
        proof -
          ...
          have "length (items (bins ?bs'' ! k)) \<ge> length (items (bins bs ! k))"
            by (metis length_nth_bin_\<pi>_it' \<pi>_it'_simps(2) a1 a2)
          hence 0: "\<not> length (items (bins ?bs'' ! k)) \<le> j"
            using \<open>i = j\<close> a1 by linarith
          have "?x = items (bins ?bs' ! k) ! j"
            using "local.1.prems"(2) a1 kth_app_bins \<open>i = j\<close> by auto
          hence 1: "?x = items (bins ?bs'' ! k) ! j"
            using kth_\<pi>_it'_bins by (metis \<open>i = j\<close> \<pi>_it'_simps(2) a1 a2 leI)
          ...
          proof -
            ...
            next
              ...
              moreover have "set_bin (bins ?bs' ! item_origin ?x) = set_bin (bins ?bs'' ! item_origin ?x)"
                using "local.1.prems"(2) set_bin_\<pi>_it'_eq calculation(1) length_bins_app_bins by presburger
              ultimately show ?thesis
                using R15 by blast
            qed
            also have "... \<subseteq> set_bin (bins ?bs' ! k)"
              using "local.1.prems"(2) app_bins_def set_bin_app_bin by auto
            also have "... \<subseteq> set_bin (bins ?bs'' ! k)"
              using "local.1.prems"(2) nth_bin_sub_\<pi>_it' by auto
            finally have "set (Complete_it k ?x ?bs'') \<subseteq> set_bin (bins ?bs'' ! k)" .
            thus ?thesis
              using app_bins_eq "1.prems"(2) length_bins_\<pi>_it' length_bins_app_bins by presburger
          qed
          ultimately show ?thesis
            by presburger
        qed
    ...
    next
      ...
      proof cases
        ...
        proof cases
          ...
          have sound: "sound_items (set_bins ?bs')"
            using sound_Scan_it[OF "1.prems"(1) _ x_in_kth_bin "1.prems"(2) a_def]
            by (metis (full_types) 0 "1.prems"(1,2,3,5) \<pi>_it'_mono sound_defs(1) sound_\<pi>_it' length_bins_app_bins subsetD)
          show ?thesis
          proof cases
            assume a5: "i+1 \<le> j"
            have "\<pi>_it' k (\<pi>_it' k ?bs' (i + 1)) j = \<pi>_it' k ?bs' (i + 1)"
          ...
          next
            ...
            proof -
              ...
              have "length (items (bins ?bs'' ! k)) \<ge> length (items (bins bs ! k))"
                by (metis "0" length_nth_bin_\<pi>_it')
              hence 0: "\<not> length (items (bins ?bs'' ! k)) \<le> j"
                using \<open>i = j\<close> a1 by linarith
              have "?x = items (bins ?bs' ! k) ! j"
                using "local.1.prems"(2) a1 kth_app_bins \<open>i = j\<close> by (simp add: app_bins_def)
              hence 1: "?x = items (bins ?bs'' ! k) ! j"
                by (metis kth_\<pi>_it'_bins \<open>i = j\<close> \<pi>_it'_simps(3) a1 a3 a4 a_def not_less)
              ...
              proof -
                ...
                also have "... \<subseteq> set_bin (bins ?bs' ! (k+1))"
                  using \<open>k + 1 < length (bins bs)\<close> app_bins_def set_bin_app_bin by auto
                also have "... \<subseteq> set_bin (bins ?bs'' ! (k+1))"
                  using "2" nth_bin_sub_\<pi>_it' by auto
                finally have "set (Scan_it k a ?x ?bs'') \<subseteq> set_bin (bins ?bs'' ! (k+1))" .
                thus ?thesis
                  using app_bins_eq \<open>k+1 < length (bins bs)\<close> length_bins_\<pi>_it' length_bins_app_bins by presburger
              qed
              ultimately show ?thesis
                by presburger
            qed
        ...
        next
          ...
          show ?thesis
            ...
            next
              assume a5: "\<not> i+1 \<le> j"
              show ?thesis
                by (smt (z3) 1 Option.option.simps(5) kth_\<pi>_it'_bins Suc_eq_plus1 \<pi>_it'_simps(1) \<pi>_it'_simps(4) a3 a4 a5 a_def le_Suc_eq le_antisym le_eq_less_or_eq not_le_imp_less)
            qed
        qed
      next
        assume a3: "\<not> is_terminal a"
        ...
        have wf: "wf_bins ?bs'"
          using "1.prems"(1,2) wf_bins_Predict_it wf_bins_app_bins x_in_kth_bin by (metis wf_bins_kth_bin wf_item_def)
        have sound: "sound_items (set_bins ?bs')"
          using sound_Predict_it by (metis 0 "1.prems"(1,2,3,5) \<pi>_it'_mono sound_\<pi>_it' length_bins_app_bins sound_items_def subsetD)
        show ?thesis
        proof cases
          assume a4: "i+1 \<le> j"
        ...
        next
          ...
          proof -
            ...
            have "?x = items (bins ?bs' ! k) ! j"
              using "local.1.prems"(2) a1 kth_app_bins \<open>i = j\<close> by auto
            hence 1: "?x = items (bins ?bs'' ! k) ! j"
              using kth_\<pi>_it'_bins \<open>i = j\<close> len by simp
            ...
            proof -
              ...
              also have "... \<subseteq> set_bin (bins ?bs' ! k)"
                using "local.1.prems"(2) app_bins_def set_bin_app_bin by auto
              also have "... \<subseteq> set_bin (bins ?bs'' ! k)"
                by (metis "local.1.prems"(2) nth_bin_sub_\<pi>_it' length_bins_app_bins)
...
next
  ...
  also have "... \<subseteq> set_bins (\<pi>_it k (\<pi>_it k bs))"
    using ABCD Suc.prems(1-3,5) wf_bins_\<pi>_it sound_\<pi>_it 0 by simp
