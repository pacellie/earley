...
lemma BUX1:
  ...
  using set_bins_bin_exists assms wf_bins_kth_bin wf_bins_def by blast

lemma AUX2:
  assumes "wf_bins bs" "k < length (bins bs)" "i \<ge> length (items (bins bs ! k))"
  shows "bin (set_bins_upto bs k i) k = bin (set_bins bs) k"
  using assms
  unfolding set_bins_upto_def set_bins_def set_bin_upto_def bin_def set_bin_def
  apply (auto simp: nth_list_update)
  apply (metis less_trans)
  apply (metis nth_mem)
  by (metis (no_types, lifting) Orderings.order_class.dual_order.strict_trans in_set_conv_nth le_eq_less_or_eq set_bin_def wf_bins_kth_bin)
...
lemma set_bin_eq_bin:
  ...
  using assms unfolding set_bins_def
  apply auto
  using wf_bins_kth_bin bin_def
  using kth_bin_in_bins apply fastforce
  using set_bins_bin_exists BUX1 bin_def set_bins_def by force
...
proof -
  ...
  also have "... = \<Union> {set_bin (bins bs ! l) |l. l < k} \<union> set_bin (bins bs ! k)"
    using assms unfolding set_bin_upto_def set_bin_def using set_take by force
...
lemma Q5:
  ...
  using Q1 Q4 assms
  by (metis set_bins_bin_exists wf_bins_kth_bin wf_items_def)
    ...
    next
      ...
      proof cases
        ...
        proof cases
          ...
          have sound: "sound_items (set_bins ?bs')"
            using sound_Scan_it[OF "1.prems"(1) _ x_in_kth_bin "1.prems"(2) a_def]
            by (metis (full_types) 0 "1.prems"(1,2,3,5) A sound_defs(1) sound_\<pi>_it' length_bins_app_bins subsetD)
          show ?thesis
          proof cases
            assume a5: "i+1 \<le> j"
            have "\<pi>_it' k (\<pi>_it' k ?bs' (i + 1)) j = \<pi>_it' k ?bs' (i + 1)"
      ...
      next
        ...
        have wf: "wf_bins ?bs'"
          using "1.prems"(1,2) wf_bins_Predict_it wf_bins_app_bins x_in_kth_bin by (metis wf_bins_kth_bin wf_item_def)
        have sound: "sound_items (set_bins ?bs')"
          using sound_Predict_it by (metis 0 "1.prems"(1,2,3,5) A sound_\<pi>_it' length_bins_app_bins sound_items_def subsetD)
        show ?thesis
        proof cases
          assume a4: "i+1 \<le> j"
...
proof (induction n)
  case 0
  thus ?case
    using A \<pi>_it_def by simp
next
  case (Suc n)
  have 0: "\<pi>_step k (set_bins_upto (\<pi>_it k bs) k 0) \<subseteq> set_bins (\<pi>_it k bs)"
    using A set_bins_upto_k0_\<pi>_it'_eq \<pi>_it_def Suc.prems(2-4) order_trans by metis
...
next
  ...
  proof -
    ...
    also have "... \<subseteq> set_bins (\<I>_it k)"
      using 1 2 set_bins_upto_sub_set_bins wf_bins_def by blast
    finally show ?thesis .
  qed
  ...
  also have "... \<subseteq> set_bins (\<pi>_it (Suc k) (\<I>_it k))"
    using \<pi>_sub_\<pi>_it[OF 1 2 3 4 5] by blast
