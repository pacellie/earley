...
typedef 'a wf_bins = "{ 
  (k, cfg::'a cfg, inp, bs, i::nat) | k cfg inp bs i.
    k < length (bins bs) \<and>
    wf_cfg cfg \<and>
    wf_bins cfg inp bs
  }"
  morphisms from_wf_bins to_wf_bins
  apply (auto simp: wf_bin_def wf_item_def)
  sorry
...
fun earley_measure :: "nat \<times> 'a cfg \<times> 'a sentence \<times> 'a bins \<times> nat \<Rightarrow> nat" where
  "earley_measure (k, cfg, inp, bs, i) = card { x | x. wf_item cfg inp x \<and> item_end x = k } - i"

function \<pi>_it' :: "'a wf_bins \<Rightarrow> 'a wf_bins" where
  "\<pi>_it' wf_bs = (
    case from_wf_bins wf_bs of
       (k, cfg, inp, bs, i) \<Rightarrow> 
          if i \<ge> length (items (bins bs ! k)) then to_wf_bins (k, cfg, inp, bs, i)
          else
            ...
            in \<pi>_it' (to_wf_bins (k, cfg, inp, bs', i+1)))"
  by pat_completeness simp
termination
  apply (relation "measure (earley_measure \<circ> from_wf_bins)")
  apply (auto)
  subgoal premises prems for wf_bs k cfg inp bs i
  proof -
    ...
    "
    have *: "k < length (bins bs)" "wf_cfg cfg" "wf_bins cfg inp bs"
      using prems(1) type_definition_wf_bins from_wf_bins by (smt (verit) mem_Collect_eq old.prod.inject)+
    ...
    ultimately have 0: "(k, cfg, inp, ?bs, Suc i) \<in> {(k, cfg, inp, bs, i) |k cfg inp bs i. k < length (bins bs) \<and> wf_cfg cfg \<and> wf_bins cfg inp bs}"
      using *(2) by blast

    have 1: "earley_measure (from_wf_bins wf_bs) = card { x | x. wf_item cfg inp x \<and> item_end x = k } - i"
      by (metis (mono_tags, lifting) earley_measure.simps prems(1))
    have 2: "earley_measure
     (from_wf_bins
       (to_wf_bins
         (k, cfg, inp, ?bs,
          Suc i))) = card { x | x. wf_item cfg inp x \<and> item_end x = k } - Suc i"
      using 0 to_wf_bins_inverse by fastforce

    have 3: "set (items (bins bs ! k)) \<subseteq> { x | x. wf_item cfg inp x \<and> item_end x = k }"
      using * wf_bin_def wf_bins_def by fastforce
    ...
    also have "... \<le> card (set (items (bins bs ! k)))"
      using prems distinct_card * wf_bins_def wf_bin_def by (metis nle_le)
