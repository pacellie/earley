...
definition sound_prered_ptr :: "'a bins \<Rightarrow> nat \<Rightarrow> 'a entry \<Rightarrow> bool" where
  "sound_prered_ptr bs k e = (\<forall>k' pre red reds r.
    PreRed (k', pre, red) reds = pointer e \<and> r \<in> set (red#reds) \<longrightarrow>
      k' < k \<and> pre < length (bs!k') \<and> r < length (bs!k) \<and>
      completes k (item (bs!k'!pre)) (item e) (item (bs!k!r)))"
      ...
      next
        ...
        proof cases
          ...
          proof (standard, standard, standard, standard, standard, standard)
            ...
            assume a4: "PreRed (K', Pre, Red) Reds = pointer elem \<and> R \<in> set (Red#Reds)"
            hence "K' = k'" "Pre = pre" "Red = red" "Reds = red'#reds@reds'"
              using a3 a4 by simp_all
            hence 2: "R \<in> set (red#red'#reds@reds')"
              using a4 by blast
            show "K' < idx \<and> Pre < length (bs[k := bin_upd e es] ! K') \<and> R < length (bs[k := bin_upd e es] ! idx) \<and>
...
proof (induction i rule: \<pi>_it'_induct[OF assms(1), case_names Base Complete Scan Pass Predict])
  ...
  hence "\<forall>e \<in> set (Complete_it k x bs i). \<forall>k' pre red reds r. PreRed (k', pre, red) reds = pointer e \<and> r \<in> set (red#reds) \<longrightarrow>
    k' < k \<and> pre < length (bs!k') \<and> r < i+1 \<and> completes k (item (bs!k'!pre)) (item e) (item (bs!k!r))"
    by metis
...
lemma A: "[x] \<in> combinations ([A]) \<Longrightarrow> x \<in> A"
  by simp
...
proof (induction bs inp k i arbitrary: t rule: build_forest'.induct)
  ...
  proof cases
    ...
    hence "build_forest' bs inp k i = FBranch (item_rule_head (item ?e)) []"
      by simp
    hence 0: "t = Branch (item_rule_head (item ?e)) []"
  ...
  next
    ...
    then obtain r where r: "r \<in> set (red#reds)" "[ts1] \<in> combinations ([trees (build_forest' bs inp k r)])"
      by (induction "red#reds") auto

    have bounds: "k' < k" "pre < length (bs!k')" "r < length (bs!k)"
      using "1.prems" prered r unfolding sound_ptrs_def sound_prered_ptr_def by (metis nth_mem)+
...
*)
    ...
    have "ts1 \<in> trees (build_forest' bs inp k r)"
      by (metis A r(2))

    have "\<forall>t \<in> trees (build_forest' bs inp k r). \<nexists>a. t = Leaf a"
      by (metis ex_Branch_build_forest' image_iff tree.distinct(1) trees.simps(2))
    then obtain N' ts' where "Branch N' ts' \<in> trees (build_forest' bs inp k r)"
      by (metis A r(2) yield_tree.cases)
    ...
    then obtain N' where "Branch N' ts1 \<in> trees (build_forest' bs inp k r)"
      using r(2) apply auto sledgehammer
    hence IH_r: "wf_item_tree cfg (item (bs!k!r)) (Branch N ts1)"
      using "1.IH"(3) "1.prems"(1-3) bounds(3) node prered r(1) by blast
