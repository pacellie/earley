...
text\<open>
In Chapter \ref{chapter:3} we proved correctness of Earley's simplified recognizer algorithm stated
abstractly in terms of a generative approach for the set Earley items. In this chapter we implement
...
specification from Chapter \ref{chapter:3} to a \textit{functional} list-based implementation and prove subsumption in both
...
information to construct parse trees, since we would otherwise have to repeat a significant amount of definitions
and proofs for Chapter \ref{chap:05}. But we only introduce and explain the needed data structures but refrain
from presenting any proofs in this chapter since constructing parse trees it is the primary subject of Chapter \ref{chap:05}.
...
@{term "|\<omega>| + 1"} containing in turn bins or variable length lists of Earley entries. An entry consists of an
...
\begin{itemize}
  ...
  \item It was scanned. Then there exists another Earley item $y$ in the previous bin $k-1$
    from which this item was generated. Hence, we keep a predecessor pointer (@{term "mbox0 (Pre pre)"})
    ...
    was created by the entry at index $1$ in $B_2$, or more precisely item $S \rightarrow \, x \bullet, 2, 2$.
  \item It was completed. Note that an item might be completed in more than one way. In each case the
    ...
    Table \ref{tab:earley-items-pointers}. This entry was first created due to the complete item $S \rightarrow \, x \bullet,4,5$
...
bin to a list of items respectively pointers it consists of.
\<close>
...
or bins, then it creates an initial bin containing entries consisting of initial items for all the
...
The list-based definitions are more specific and replace the argument $I$ in the following ways: function
...
is slightly more involved. It takes as arguments again $x$ and the index @{term red} of $x$ (it is a
reduction item this time) in the current bin $k$, but also the complete bins @{term bs}, since
it needs to find all potential predecessors items as well as their indices in the origin bin of $x$ (see
...
multiple entries into a specific bin. An alternative equivalent implementation would be @{term "fold bin_upd es b"}.
...
text\<open>
Draft:
Explain Earley bin which is Earley step + function iteration is now Earley bin list(').
\<close>
