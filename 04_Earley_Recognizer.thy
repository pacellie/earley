...
an executable implementation. But instead of re-proving soundness and completeness
...
run out of items to operate on; one case for completion (@{term Complete}) and prediction (@{term Predict}) each; and two cases for scanning
...
fixpoint iteration of @{term Earley_step} that is in turn defined as @{term "I \<union> Scan k \<omega> I \<union> Complete k I \<union> Predict k \<G> I"}.
