...
text\<open>
In Chapter \ref{chapter:3} we proved correctness of Earley's simplified recognizer algorithm implemented
abstractly by an operational approach for generating the set of Earley items. In this chapter we implement
an executable version of the recognizer algorithm. But instead of re-proving soundness and completeness
...
\begin{itemize}
  \item It was predicted. In that case we consider it created from thin air and do not need to
    ...
    $B_0$ contains the entry $S \rightarrow \, \bullet x, 0, 0; \bot$ consisting of the item $S \rightarrow \, \bullet x, 0, 0$ and a null pointer denoted by $\bot$.
  \item It was scanned. Then there exists another Earley item $y$ in the previous bin $k-1$
    from which this item was computed. Hence, we keep a predecessor pointer (@{term "mbox0 (Pre pre)"})
    where @{term pre} is a natural number indicating the index of item $y$ in bin $k-1$. Bin $B_3$ contains in Table \ref{tab:earley-items-pointers}
    the entry $S \rightarrow \, x \bullet, 2, 3; 1$, the predecessor pointer is $1$ since this item
    was created by the the item $S \rightarrow \, x \bullet, 2, 2$ of the entry at index $1$ in $B_2$.
  \item It was completed. Note that an item might be completed in more than one way. In each case the
    ...
    (@{term "PreRed (k', pre, red) reds"}) where $k'$, $pre$, and $red$ are respectively the index of the origin bin of
    ...
    Table \ref{tab:earley-items-pointers}. This entry (without the second reduction triple) was first created due to the complete item $S \rightarrow \, x \bullet,4,5$
...
conceptual bin, the grammar @{term \<G>}, the input @{term \<omega>}, and the current set of Earley items $I$.
...
only needs access to the next non-terminal symbol $N$ of $x$, and returns only entries with null pointers. The implementation of @{term Complete_list}
...
that updating a bin (@{term bin_upd}) or inserting an additional entry into a bin maintains its set properties: the set
of items of a bin should be distinct. Additionally, since it is possible to generate multiple
reduction pointers for the same item, we have to take care to update the pointer information accordingly
if the item of the entry to be inserted matches the item of an already present item. Function @{term bin_upds} inserts
...
text\<open>
Function @{term Earley_bin_list'} completes the $k$-th bin starting from index $i$. It updates the
bins @{term bs} using function @{term bins_upd} and the appropriate operation depending on the next symbol of
the item under consideration (@{term "x = items (bs!k)!i"}) which can either be some terminal or
non-terminal symbol or @{term None}. We have to define the function as a @{term partial_function},
since it might never terminate if it keeps appending newly generated items to the $k$-th bin it operates
on. We prove termination and highlight the relevant Isabelle specific details in Section \ref{sec:04-wellformedness}.
Intuitively, the function terminates since it generates a subset of the set of Earley items that
is finite by lemma @{term finite}. Function @{term Earley_bin_list} fully completes the $k$-th bin and
thus corresponds to the function @{term Earley_bin}.
\<close>
...
@{term Earley} respectively @{term \<E>arley}, differing only in the type of the used operations and the
return type.
\<close>
...
argument for the running time which is $\mathcal{O}(n^3)$ where @{term "n = |\<omega>|"}. Our implementation
is purely functional, and we might expect a quite significant decrease in performance. In this section we
...
implementation which achieves a theoretical running time of $\mathcal{O}(n^3 \log{n})$, and highlight possible further
...
from $n$ and can thus be considered (possible large) constants. The origin $i$ is $0 \le i \le j$ and thus
depends on $j$ which is in turn bounded by $n$. Overall the number of items in each bin $B_j$ is $\mathcal{O}(n)$.

We have @{term Init_list} $\in \mathcal{O}(n)$ since the function @{term replicate} takes time linear in the length of @{term \<omega>},
and functions @{term filter} and @{term map} operate at most on the size of the grammar @{term \<G>}.
...
at most one new entry. Function @{term Predict_list} take time in the the size of the grammar @{term \<G>},
due to the @{term filter} and @{term map} functions, and computes at most @{term "|\<G>|"} new items.
Function @{term Complete_list} again takes linear time, since finding the origin bin of the given item
$x$ (@{term "bs!item_origin x"}) takes linear time, and functions @{term items}, @{term filter_with_index}, and @{term map}
operate on the origin bin which is of at most linear size. And consequently the function also computes at most
$\mathcal{O}(n)$ new items. 

Updating a bin (@{term bin_upd}) with a single entry takes at most linear time, inserting $e$ new entries
(@{term bin_upds}) consequently takes time $e \cdot \mathcal{O}(n)$, and hence function @{term bins_upd}
also runs in time $e \cdot \mathcal{O}(n)$. The analysis of function @{term Earley_bin_list'} is slightly
more involved. Since it computes the contents of a bin $B_k$, it calls itself recursively at most $n$ times.
The time for one function execution is dominated by the time it takes to update the bins with the newly
created items whose number depends on in turn on the operation we applied but is bounded in the worst case by $n$ (@{term Complete_list}).
All the other operations of the function body run in linear time. Overall we have for the body of @{term Earley_bin_list'} $= \mathcal{O}(n) + e \cdot \mathcal{O}(n) = \mathcal{O}(n^2)$.
...
bins which takes linear time and not constant time. Earley's implementation implements the algorithm as
follows. On the top-level bins are no longer a list but an array. Each bin is a single linked list, and
pointers are no longer represented by the type @{term pointer} but by actual pointers between Earley items.
...
way is to not only keep a singly-linked list of items and pointers but additionally a map whose key are the items of the
list and stores for a specific item a pointer to its position in the list. Insertion of a new item
into a bin works then as follows: if the item is already present in the map, we follow the pointer and
update the pointers in the list accordingly depending on the kind of item, otherwise we just append the
item and pointers to the list and insert the item and a pointer to it into the map.

TODO:
Functional setting does not work: pointer of the map will be an index, and thus we still need to index in linear time
and not constant time follow the pointer. For a recognizer it would work since we don't need to merge pointers
and only need to keep a set instead of a map. decrease from n to log n in bin upd. There is another way to
achieve functional for n3 log n. keep a tree map of items to pointers and adapt the pointer data type
to not store indices but Null, Pre item, (PreRed item item) list, consider then this map the actual bin
and the list will only be a worklist where we don't append to by cons onto.

TODO:
other not necessary performance improvements:
complete faster by keeping a map from nonterminal which are next in the items to the actual items
init and predict faster by organizing the grammar in an efficient manner by nonterminal
introduce a lookahead of 1
